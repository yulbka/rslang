...
export class Puzzle {
  ...
  render() {
    ...
    const wrapper = createElement('div', data, ['puzzle-wrapper', 'new'], '', 'data-word', this.text);
    wrapper.dataset.position = this.position;
    if (this.type !== 'last') {
      wrapper.style.width = `${this.width - 20}px`;
    }
    const puzzle = createElement('canvas', wrapper, ['puzzle'], undefined, 'height', '50');
  ...
  static showPicture(element, position, rowPosition, level, page, color) {
    ...
    img.onload = () => {
      ...
      ctx.drawImage(
        img,
        position / widthScale,
        (rowPosition * 50) / heightScale,
        canvas.width / widthScale + 2,
        50 / heightScale + 2,
        0,
        0,
        canvas.width,
      ...
      color ? Puzzle.paintStroke(element, color) : Puzzle.paintStroke(element, 'lightgrey');
    };
  }
}
