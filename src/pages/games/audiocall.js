...
function playAudiocallGame() {
    ...
    function renderContent() {
        ...
        return `
            <img class="card-img" src='https://raw.githubusercontent.com/MariannaV/rslang-data/master/${guessWord.image}' />
           ...
           <div class="block-with-words">
            ${answers.map(({word, wordTranslate}, index) => `<p class="answer-word" data-id=${word}><span>${index + 1}</span>${wordTranslate}</p>`).join('')}
           </div>
          `
    }
